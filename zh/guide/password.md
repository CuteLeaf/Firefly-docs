...
在文章的 Front-matter 中添加 `password` 字段即可启用加密：
...
可选添加 `passwordHint` 字段，为访客提供密码提示：
...
| 属性 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `password` | `string` | 否 | 文章密码，设置后文章将被加密 |
...
- 输入正确密码后，密码会缓存在浏览器 `sessionStorage` 中
- 同一会话内刷新页面无需重复输入密码
